...
void print_usage()
{
  printf("Usage for talker1 app:\n");
  printf("talker1 [-t topic_name] [-h]\n");
  printf("options:\n");
...
class Talker1 : public rclcpp::Node
{
public:
  explicit Talker1(const std::string & topic_name)
  : Node("talker123")
  {
...
int main(int argc, char * argv[])
{
  ...
  // Create a node.
  auto node = std::make_shared<Talker1>(topic);

  // spin will block until work comes in, execute work as it becomes available, and keep blocking.
  // It will only be interrupted by Ctrl-C.
  printf("start rclcpp::spin(node)\n");
  rclcpp::spin(node);
  printf("end rclcpp::spin(node)\n");
  printf("start rclcpp::shutdown()\n");
  rclcpp::shutdown();
